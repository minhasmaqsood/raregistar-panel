...
import durationPicker from 'html-duration-picker'
...
export default class CreateQuiz extends Component {
	...
	handleQuestionImage = (image, index) => {
		let questions = this.state.questions
		questions[index].value = ''
		let questionImages = this.state.questionImages
		let imagePresence = questionImages.filter(({ key, value }) => key === `q${index}`)
	...
	addQuestionsListInput = () => {
		...
		let questions = { 'type': '', 'duration': { hours: 0, minutes: 0, seconds: 30 }, name: `q${questionLength}`, 'options': [] };
	...
	handleOptionsSelectedType = (e, index) => {
		...
		if (e.target.value !== 'null') {
			if (e.target.value === 'text') {
				let questionOptionImages = this.setState.questionOptionImages
				questionOptionImages.splice(index, 1)
				this.setState({
					questionOptionImages: questionOptionImages
				})
			}
									...
									<CardBody>
										...
										<Form className="dashboard-quick-post" onSubmit={this.createQuiz}>
											...
											<Button
												...
												<span className="label"><IntlMessages id="quiz.create" /></span>
											</Button>
										</Form>
									</CardBody>
								</Card>
