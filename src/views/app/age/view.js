...
export default class AgeView extends Component {
    ...
    confirmChangeStatus = async (id) => {
        this.setState({spinning: true});
        let response = await ApiCall.get(`${Url.DELETE_AGE}/${id}`, await config());
        if(response.status === 200){
    ...
    getData =() => {
        ...
        return this.state.ages.filter((v, i) => {
                ...
                <Row>
                    <Colxx xxs="12">
                        ...
                        <div className="text-zero top-right-button-container">
                            <Link to='/app/age/create'><Button size='lg' color={'secondary'}><IntlMessages id={"menu.create"} /></Button></Link>
                        </div>
                                            ...
                                            {(rowData, rowIndex) => {
                                                return <div>
                                                    <Button color="secondary" size="xs" className="mb-2">
                                                        <Link to={`/app/age/edit/${rowData._id}`} style={{color: 'white'}}><IntlMessages id="edit" /></Link>
                                                    </Button>
                                                    {" "}{" "}
