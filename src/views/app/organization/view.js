...
console.log(response)
    ...
    confirmChangeStatus = async (id) => {
        this.setState({spinning: true});
        let response = await ApiCall.get(`${Url.DELETE_ORGANIZATION}/${id}`, await config());
        if(response.status === 200){
    ...
    getData =() => {
        ...
        return this.state.organizations.filter((v, i) => {
                                                ...
                                                {(rowData, rowIndex) => {
                                                    return <div>
                                                        <Button color="secondary" size="xs" className="mb-2">
                                                            <Link to={`/app/organization/edit/${rowData._id}`} style={{color: 'white'}}><IntlMessages id="edit" /></Link>
                                                        </Button>
                                                        {" "}{" "}
