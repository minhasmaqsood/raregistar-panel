...
export default class CourseTranslations extends Component {
    ...
    getCourseVideos = async ()=> {
        ...
        const vidId = parseInt(this.props.match.params.id)
        if(!this._isMounted){
            let response = await ApiCall.get(Url.GET_COURSE_VIDEOS, await config())
            // return console.log(response)
            if(response.status=== 200){
                const translations = response.data.videos.filter(item => item.id === vidId).length> 0?
                    response.data.videos.filter(item => item.id === vidId)[0].translations : []
                this.setState({
                        ...
                        <Card className="h-100">
                            <CardBody>
                                ...
                                <TranslationsModal
                                    ...
                                    onCourseVideoComplete={this.onCourseVideoComplete}
                                    modalMethod={"store"}
                                />
                                }
                            </CardBody>
